...
const EmployeeHomePage = () => {
  ...
  const fetchShifts = async () => {
    try {
      ...
      const active = formattedShifts.find(
        (shift) =>
          shift.status === "active" ||
  ...
  useEffect(() => {
    ...
    const calculateTimeElapsed = () => {
      try {
        // Используем Luxon для работы с временем в нужной временной зоне
        const now = DateTime.now().setZone("Asia/Almaty").minus({ hours: 1 });

        let startDateTime;

        if (activeShift.checkInTime) {
          // Если сотрудник отсканировал QR-код, используем это время
          startDateTime = DateTime.fromISO(activeShift.checkInTime).setZone(
            "Asia/Almaty",
          );
        } else {
          // Если смена еще не начата фактически, используем запланированное время
          const [hoursStart, minutesStart] = activeShift.startTime
            .split(":")
            .map(Number);

          // Создаем дату начала смены с правильным часовым поясом
          startDateTime = DateTime.fromISO(activeShift.date).setZone(
            "Asia/Almaty",
          );
        ...
        // Расчет разницы с использованием API Luxon
        const diff = now.diff(startDateTime, ["hours", "minutes", "seconds"]);
    ...
    // Аналогично нужно обновить функцию calculateTimeLeft
    const calculateTimeLeft = () => {
      try {
        const now = DateTime.now().setZone("Asia/Almaty").minus({ hours: 1 });

        // Получаем часы и минуты окончания смены
        const [hoursEnd, minutesEnd] = activeShift.endTime
          .split(":")
          .map(Number);

        // Создаем дату окончания смены с правильным часовым поясом
        const endDateTime = DateTime.fromISO(activeShift.date)
          .setZone("Asia/Almaty")
          .set({
            hour: hoursEnd,
            minute: minutesEnd,
        ...
        // Расчет разницы с использованием API Luxon
        const diff = endDateTime.diff(now, ["hours", "minutes", "seconds"]);
